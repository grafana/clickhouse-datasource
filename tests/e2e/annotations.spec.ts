...
// Grafana renders InlineFormLabel + Select inside one .gf-form row; scope the
// combobox to the row that carries the given label text.
const comboFor = (page: Page, label: string) => page.locator('.gf-form', { hasText: label }).getByRole('combobox');
...
test.describe('annotation editor', () => {
  ...
  test('change detection preset reveals the schema builder', async ({
    ...
    await expect(comboFor(page, 'Watch Column')).toBeVisible();
