...
- Add logs data source options to show table columns as fields: **Include all columns as fields** (a toggle) and **Additional fields** (a list). The selected columns appear in the Fields list, the log row details (under a **Fields** group next to the attribute groups), and the field filters, without editing each query (#2086)
