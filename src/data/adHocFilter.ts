...
export class AdHocFilter {
  ...
  apply(sql: string, adHocFilters: AdHocVariableFilter[], useJSON = false): string {
    ...
    // the result is intentionally not cached back onto the instance: caching it
    // would let the first panel to render decide the table for every later panel.
    const targetTable = this._targetTable !== '' ? this._targetTable : getTable(sql);
    ...
    if (!sql.replace(/["`]/g, '').match(new RegExp(`.*\\b${escapeRegExp(targetTable)}\\b.*`, 'gi'))) {
      return sql;
    }
