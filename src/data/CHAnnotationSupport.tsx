...
export function generateChangeDetectionSQL(opts: ChangeDetectionState): string {
  ...
  if (mapKey) {
    // buildJSONPathAccess takes the raw column name: it quotes the column itself,
    // backtick-escapes the path segments and applies the Nullable(String) cast.
    // That cast matters because JSON path access returns Dynamic, so topK and
    // concat need strings, and rows without the path become NULL and fail the
    // != '' guard, just as rows missing a map key do.
    columnExpr = isJsonColumn
      ? buildJSONPathAccess(column, mapKey)
