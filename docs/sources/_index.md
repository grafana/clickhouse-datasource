...
- **OTel Logs Explorer**: stacked log volume, services, and per-service samples for OpenTelemetry data exported to `otel_logs`. See the [OpenTelemetry dashboards guide](/docs/plugins/grafana-clickhouse-datasource/<CLICKHOUSE_PLUGIN_VERSION>/dashboards/).
- **OTel Traces Explorer**: service map, trace search, and per-service performance for OpenTelemetry data exported to `otel_traces`.
- **OTel Service Dashboard**: single-service deep dive with RED metrics, top errors, slowest operations, related logs, and trace detail.
